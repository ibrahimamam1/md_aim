...
class DisableBootstrapCallback(BaseCallback):
    """
    Prevents SB3 from crashing when computing terminal bootstraps with dual value heads.
    By setting TimeLimit.truncated to False in the infos dict, SB3 skips the problematic
    scalar addition block completely.
    """
    def _on_step(self) -> bool:
        for info in self.locals.get("infos", []):
            if "TimeLimit.truncated" in info:
...
def train():
    ...
    disable_bootstrap_cb = DisableBootstrapCallback()
